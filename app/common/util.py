...
def abort_if_user_does_not_exist(email):
    ...
    if user == False:
        return response("user with email {0} doesn't exist".format(email), 404)

def abort_if_email_does_not_match_type_email(email):
    if not re.match(r"[^@]+@[^@]+\.[^@]+", email):
        return response("missing or incorrect email format", 400)
        # abort(400)
        # TODO: Get a better solution to handle exceptions

def abort_if_password_is_less_than_4_characters(password):
    if (len(str(password)) < 4):
        return response("password is missing or less than 4 characters", 400)
...
def abort_if_user_does_not_have_orders(email):
    if (len(parcel_delivery_orders[email]) == 0):
        return response("User with email {0} does not have any orders".format(email), 404)
