...
def abort_if_email_does_not_match_type_email(email):
    if not re.match(r"[^@]+@[^@]+\.[^@]+", email):
        abort(make_response(jsonify(
            message="missing or incorrect email format"),
            400))
...
def abort_if_parcel_does_not_exist(orderId):
    ...
    if parcel == False:
        abort(make_response(jsonify(
            message="parcel order with id {0} does not exist".format(orderId)),
            400))
...
def abort_if_attribute_is_empty(attribute, value):
    if value == "" or not value:
        abort(make_response(jsonify(
            message="attribute {0} or its value is missing".format(attribute)),
            400))
...
def abort_if_parcel_input_is_missing(parameter):
    parcel_details = ["parcel", "weight", "price", "receiver",
                      "pickup_location", "destination"]
    user_provided_attributes = parameter.keys()
    missing_attributes = list(set(parcel_details) -
                              set(user_provided_attributes))
    if len(missing_attributes) > 0:
        abort(make_response(jsonify(
            message="attribute(s): {0} are missing \
...
def send_mail(mail_to, message):
    ...
    password = getenv('ADMIN_MAIL_PASSWORD')
    mailServer = SMTP_SSL('smtp.gmail.com', 465)
    mailServer.login(admin_mail, password)
    subject = 'SEND IT COURIER SERVICES'
    body = '\r\n'.join(['To: %s' % mail_to,
                        'From: %s' % admin_mail,
                        'Subject: %s' % subject,
                        '', message])
    mailServer.sendmail(admin_mail, mail_to, body)
    mailServer.quit()
